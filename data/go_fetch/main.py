...
def main():
    ...
    # Get data from market_price_usd function
    # test = market_price_usd()

    test = market_volume_usd()
    if not test:
    ...
    try:
        ...
        # Write to CSV file
        with open('BTC-FundData-MarketVolume.csv', 'a', newline='') as csvfile:
            # Get the fieldnames from the first item in the data
            if isinstance(data, dict) and 'data' in data and data['data']:
                fieldnames = data['data'][0].keys()
            else:
        ...
        print("Data has been written to BTC-FundData-MarketVolume.csv")
    except Exception as e:
        print(f"Error processing data: {e}")
        return
